...
base_output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "baseline_well_mixed_sh")
...
def run_baseline_well_mixed():
    ...
    # Exports CSV data.
    sh.DataFrame({"t": time_steps, "Cooperation_Rate": cooperation_rates, "Defection_Rate": defection_rates}).to_csv(os.path.join(csv_dir, "cooperation_defection_actions.csv"), index=False)
    sh.DataFrame({"t": time_steps, "CC_Rate": cc_rates, "CD_Rate": cd_rates, "DD_Rate": dd_rates}).to_csv(os.path.join(csv_dir, "state_densities.csv"), index=False)
    sh.DataFrame({"t": time_steps, "Cooperators_Reward": coop_avg_rewards, "Defectors_Reward": defect_avg_rewards}).to_csv(os.path.join(csv_dir, "avg_rewards_by_strategy.csv"), index=False)
