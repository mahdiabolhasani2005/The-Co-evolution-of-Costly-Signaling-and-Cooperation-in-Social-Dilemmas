...
def run_simulation(params):
    ...
    for gen in range(rounds):
        ...
        # Random game selection (PD or SH) for each edge
        ...
        r1_d = np.where(use_pd_down, payoff_PD[a1_d, a2_d], payoff_SH[a1_d, a2_d])
        r2_d = np.where(use_pd_down, payoff_PD[a2_d, a1_d], payoff_SH[a2_d, a1_d])
        ...
        r1_r = np.where(use_pd_right, payoff_PD[a1_r, a2_r], payoff_SH[a1_r, a2_r])
        r2_r = np.where(use_pd_right, payoff_PD[a2_r, a1_r], payoff_SH[a2_r, a1_r])
...
def plot_transition_graph(df, save_path):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, "PD_SH_Transition_Plot.png"), dpi=300)
    plt.close('all')
