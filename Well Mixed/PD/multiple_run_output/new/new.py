...
# جمعیت اولیه
# هزینه سیگنال‌ها (ثابت در طول شبیه‌سازی)
...
for gen in range(rounds):
    ...
    if gen % 100 == 0:
        ...
        if gen % 1000 == 0:
            print(f"Generation {gen}: Cooperation = {coop_rate:.3f}")

# --- پردازش نهایی داده‌ها ---

# 1. محاسبه ایندکس‌های مرتب‌سازی بر اساس هزینه
# ایندکس‌هایی را پیدا می‌کنیم که سیگنال‌ها را از کم‌هزینه به پرهزینه مرتب کنند
sorted_indices = np.argsort(signal_costs)
sorted_costs = signal_costs[sorted_indices]

# داده‌های اصلی برای رسم هیت‌مپ‌ها باید ستون‌هایشان جابجا شود
signal_usage_array = np.array(signal_usage_over_time)
coop_strategy_array = np.array(coop_strategy_over_time)
signal_rewards_array = np.array(signal_rewards_over_time)

# اعمال مرتب‌سازی روی ستون‌ها (محور سیگنال‌ها)
sorted_signal_usage = signal_usage_array[:, sorted_indices]
sorted_coop_strategy = coop_strategy_array[:, sorted_indices]
sorted_signal_rewards = signal_rewards_array[:, sorted_indices]
...
# >> نمودار جدید: هزینه واقعی در برابر استفاده <<
plt.figure(figsize=(10, 6))
plt.scatter(signal_costs, avg_signal_usage, color='teal', alpha=0.7, edgecolors='k')
plt.xlabel("Signal Cost (Real Value)")
plt.ylabel("Average Signal Usage")
plt.title("Signal Usage vs. Signal Cost")
plt.grid(True, linestyle='--', alpha=0.6)
plt.savefig(os.path.join(images_dir, "real_cost_vs_usage.png"), dpi=300)
plt.show()

# ذخیره CSV جدید برای نمودار بالا
df_cost_usage = pd.DataFrame({
    "Signal_Index_Original": range(n_signals),
    "Cost": signal_costs,
    "Average_Usage": avg_signal_usage
})
# برای خوانایی بهتر در CSV، آن را هم بر اساس هزینه مرتب می‌کنیم
df_cost_usage_sorted = df_cost_usage.sort_values(by="Cost")
df_cost_usage_sorted.to_csv(os.path.join(csv_dir, "signal_cost_vs_usage.csv"), index=False)
print(f"New CSV saved: signal_cost_vs_usage.csv")


# نمودارهای قبلی (بدون تغییر)
plt.figure()
...
# --- هیت‌مپ‌های اصلاح شده (مرتب‌سازی شده بر اساس هزینه) ---

# 1. Heatmap: Signal Usage (Sorted by Cost)
plt.figure()
plt.imshow(sorted_signal_usage.T, cmap='viridis', aspect='auto', origin='lower')
plt.colorbar(label='Usage Probability')
plt.xlabel("Generation")
plt.ylabel("Signal Index (Sorted by Cost: Low -> High)") # لیبل تغییر کرد
plt.title("Signal Usage Over Time (Sorted by Cost)")
plt.savefig(os.path.join(images_dir, "signal_usage_heatmap_sorted.png"), dpi=300)
plt.show()

# 2. Heatmap: Cooperation Strategy (Sorted by Cost)
plt.figure()
plt.imshow(sorted_coop_strategy.T, cmap='YlGnBu', aspect='auto', origin='lower')
plt.colorbar(label='C Probability')
plt.xlabel("Generation")
plt.ylabel("Signal Index (Sorted by Cost: Low -> High)")
plt.title("Cooperation Strategy Over Time (Sorted by Cost)")
plt.savefig(os.path.join(images_dir, "coop_strategy_heatmap_sorted.png"), dpi=300)
plt.show()

# 3. Heatmap: Signal Rewards (Sorted by Cost)
plt.figure()
plt.imshow(sorted_signal_rewards.T, cmap='magma', aspect='auto', origin='lower')
plt.colorbar(label='Avg Reward')
plt.xlabel("Generation")
plt.ylabel("Signal Index (Sorted by Cost: Low -> High)")
plt.title("Signal Average Rewards Over Time (Sorted by Cost)")
plt.savefig(os.path.join(images_dir, "signal_rewards_heatmap_sorted.png"), dpi=300)
plt.show()

# --- ذخیره سایر فایل‌های CSV ---
results_summary = {
...
pd.DataFrame([results_summary]).to_csv(os.path.join(csv_dir, "average_rates_summary.csv"), index=False)

# ذخیره داده‌های مرتب شده در CSVهای جداگانه (برای راحتی تحلیل)
pd.DataFrame(sorted_signal_usage, columns=[f"Signal_Rank_{i}_Cost_{c:.3f}" for i, c in enumerate(sorted_costs)])\
  .to_csv(os.path.join(csv_dir, "signal_usage_over_time_sorted.csv"), index=False)

pd.DataFrame(sorted_coop_strategy, columns=[f"Signal_Rank_{i}_Cost_{c:.3f}" for i, c in enumerate(sorted_costs)])\
  .to_csv(os.path.join(csv_dir, "coop_strategy_over_time_sorted.csv"), index=False)

# ذخیره سایر داده‌های خام (بدون سورت، مثل قبل)
...
print(f"\n✅ Simulation complete. Heatmaps are now sorted by cost (Low->High).")
print(f"✅ New CSV 'signal_cost_vs_usage.csv' created.")
print(f"✅ All data saved in: {csv_dir}")