...
def run_simulation(params):
    ...
    for gen in range(rounds):
        ...
        r1 = np.where(is_pd, payoff_PD[a1, a2], payoff_SH[a1, a2])
        r2 = np.where(is_pd, payoff_PD[a2, a1], payoff_SH[a2, a1])
    ...
    print(f"پایان اجرا: {run_name}. زمان: {time.time() - start_time:.2f} ثانیه.")
    return steady_state
...
def plot_transition_graph(df, save_path):
    ...
    plt.title('PD-SH Transition (Well-Mixed)')
    plt.xlabel('$P_{PD}$')
